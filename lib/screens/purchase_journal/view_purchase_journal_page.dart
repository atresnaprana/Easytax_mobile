...
class _ViewPurchaseJournalEntryPageState
  ...
  Future<void> _fetchAllAccountsIfNeeded() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception('Failed to load account list.');
      }
    } catch (e) {
  ...
  Future<void> _fetchEntryAndAccountDetails() async {
    ...
    try {
      ...
      final url = Uri.parse('$baseUrl/api/API/ViewJPB?id=${widget.entryId}');
      final headers = {'Authorization': 'Bearer $token'};
      final response = await http
      ...
      if (response.statusCode == 200) {
        ...
        final fetchedEntry = PurchaseJournalDetail.fromJson(data);
  ...
  Widget build(BuildContext context) {
    ...
    if (_fetchError != null || _entry == null) {
      return Scaffold(
        ...
        body: Center(
          child: Text('Error: ${_fetchError ?? "Entry not found."}'),
        ),
      );
    }
